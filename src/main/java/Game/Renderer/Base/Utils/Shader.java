...
public class Shader {
    ...
    public void addUniform2i(String uniformName, int[] value){

        int uniformLocation = GL20.glGetUniformLocation(ID, uniformName);


        if (uniformLocation != -1) {
            GL20.glUniform2i(uniformLocation, value[0], value[1]);
        }
        else { uniformErrorMessage(uniformName);}
    }
    ...
    public void addUniform4fv(String uniformName, Vector4f vector) {
        int uniformLocation = GL20.glGetUniformLocation(ID, uniformName);
        if (uniformLocation != -1) {

            FloatBuffer buffer = BufferUtils.createFloatBuffer(4);
            buffer.put(vector.x).put(vector.y).put(vector.z).put(vector.w);
            buffer.flip();

            GL20.glUniform4fv(uniformLocation, buffer);
        } else {
            System.err.println("Uniform " + uniformName + " not found in shader program.");
        }
    }
